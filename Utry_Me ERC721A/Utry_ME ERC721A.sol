...
contract UTry is ERC721A {
    address public owner;
uint public maxMint=1000;
using SafeMath for uint;
 modifier onlyOwner {
    ...
    Counters.Counter public count;
     string public baseURI="https://gateway.pinata.cloud/ipfs/QmeaB5b1owqgNxUCcb8p42QsbsTdnxYGazhRaftB9np8pe/";
  string public baseExtension = ".json";
    constructor() ERC721A("UTry.me", "UTRY") {
      owner=msg.sender;
...
function updateMaxMint(uint _maxMint) public onlyOwner{
  ...
  function _baseURI() internal view virtual override returns (string memory) {
    ...
    if(ownerOf(_id[i])==owner)
    {

            safeTransferFrom(owner, _to[i], _id[i]);
            count.increment();
            }
    
     else if (ownerOf(_id[i])!=owner)
     { uint j=i;

          count.increment();

          safeTransferFrom(owner, _to[i], _id[j]);
          count.increment();
